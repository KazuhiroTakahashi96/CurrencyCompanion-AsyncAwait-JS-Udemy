...
async function getExchangeRate(fromCurrency, toCurrency) {
  ...
  const currencyDate = await response.json();
  const currencyRates = currencyDate.rates;
  const baseCureency = 1 / currencyRates[fromCurrency];
  const exchangeRate = baseCureency * currencyRates[toCurrency];
...
async function convertCurrency(fromCurrency, toCurrency, exchangeAmount) {
  ...
  return `$(convertedAmount) ${fromCurrency} ===> ${convertedAmount} ${toCurrency}`;
}
